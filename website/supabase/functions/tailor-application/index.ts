...
Background in <the candidate's actual field, in that field's own words>, with strengths in <two capabilities the EXPERIENCE section evidences>. <strongest outcome with its figure>; <second outcome with its figure>.

Worked example: "Background in regulatory reporting and risk analytics, with strengths in stakeholder management and process improvement. Rebuilt a credit risk reporting suite for a GBP 2.6bn portfolio; cut month-end close from nine working days to three."

WHY THIS SHAPE AND NOT A TITLE. An earlier version opened with a held job title ("Solutions Architect working across ..."). On an application to another field that announces the wrong profession in the first line a screener reads, and on a pivot the held title is the least relevant true thing on the page. Naming the FIELD and the STRENGTHS says what transfers without claiming a job that was never held. A candidate applying within their own field may still name the field plainly.

NEVER IN THE SUMMARY:
- A job title the candidate has not held. The headline under the name carries the target role; the summary never borrows the posting's title.
...
DO NOT WRITE A CLOSING SENTENCE NAMING THE TARGET ROLE. A later pass adds one, and two of them read as a draft nobody finished.

THE STRENGTHS CLAUSE: two capabilities in the POSTING'S OWN WORDING, and only where a PROFESSIONAL EXPERIENCE bullet evidences them. A term appearing only in the skills list is a word on a page, not work done. If fewer than two qualify, write NO strengths clause at all and let the outcomes carry the sentence. Never pad it.
...
RELEVANCE BEATS SIZE. Choose the outcomes the POSTING would care about, not the biggest numbers on the CV. A coaching posting is better served by "trained 24 analysts across two offices" than by a batch job going from six hours to one, however much larger the second figure is.
...
ABSOLUTE RULES:
1. PROFESSIONAL SUMMARY - REQUIRED SHAPE, two sentences, 150 to 220 characters total: "Background in <the candidate's actual field>, with strengths in <two capabilities the EXPERIENCE section evidences>. <strongest outcome with its figure>; <second outcome with its figure>." Example: "Background in regulatory reporting and risk analytics, with strengths in stakeholder management and process improvement. Rebuilt a credit risk reporting suite for a GBP 2.6bn portfolio; cut month-end close from nine working days to three." Naming the field rather than a held title keeps the first line relevant on a pivot without claiming a job never held.
1a. NEVER IN THE SUMMARY: a title never held (the headline under the name carries the target role; the summary never borrows the posting's title), employer names, total years of experience, place names, any adjective describing the candidate (accomplished, seasoned, passionate, dynamic, results-driven, proven track record, strong background, operational excellence), any self-rating (expert, world-class, exceptional), first person, "seeking"/"looking for", or tools/technologies/skill runs. Do NOT write a closing sentence naming the target role - a later pass adds one. Strength terms must be in the posting's wording AND evidenced by an experience bullet, not the skills list; if fewer than two qualify, omit the strengths clause entirely rather than pad it. Two outcomes joined by a SEMICOLON, never two sentences; figures exactly as the profile states them - never rounded, never dropped, never invented; prefer a before-and-after, then a magnitude, then a percentage, then a count; and choose the outcomes the POSTING cares about over the biggest numbers on the CV.
