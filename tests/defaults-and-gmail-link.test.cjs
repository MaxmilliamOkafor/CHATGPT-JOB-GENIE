...
// THREE THINGS REACH A REAL EMPLOYER AND CANNOT BE TAKEN BACK.
//
// Easy Apply autofill, pressing Submit, and sending a follow-up email
// are each irreversible and outward-facing -- and a follow-up lands in
// an inbox that already holds every previous application to that
// company, where a duplicate reads as spam. All three are opt-in, and
// an unset preference means no. Only auto-ADVANCE ships on, and it
// cannot act at all until Easy Apply autofill is switched on first.
const OPT_IN = ['linkedin_autofill_enabled', 'linkedin_autosubmit_enabled', 'followup_enabled'];
...
for (const k of OPT_IN) {
  ...
  t(k + ' is read with === true, so unset means off',
    new RegExp('\\b' + k + '\\s*===\\s*true').test(bg + popupJs + core)
      || new RegExp('\\b' + k + '\\s*!==\\s*true').test(bg + popupJs + core),
    'a reader would treat unset as on');
}
...
for (const [id,key] of [['subToggle','linkedin_autosubmit_enabled'],['fuToggle','followup_enabled']]) {
  t('the popup shows ' + key + ' OFF until switched on',
    new RegExp(id + '\\.checked = result\\.' + key + ' === true').test(popupJs),
    'the switch would draw ON while the action is armed');
}
...
t('the auto-send gate treats unset as OFF',
  /cfg\.followup_enabled !== true/.test(popupJs), 'would email a recruiter on a fresh install');
