...
// A PERSONAL ADDRESS IS NOT AUTOMATICALLY BETTER THAN NOTHING.
//
// This used to return a scraped gmail address as the chosen recipient
// with a `personal` flag on it -- and the auto-send path then emailed
// it. A follow-up to a personal inbox, from an address the recruiter
// never gave out, reads as exactly what it is. It is surfaced for
// review now instead: the address is carried in `alternatives`, nothing
// is chosen, and requiresReview says why.
RESPOND=()=>reply(200,{data:{entries:[
...
t('a personal-only result is never auto-chosen', be.email==='', JSON.stringify(be));
t('...but it is surfaced for review rather than discarded',
  be.requiresReview===true && (be.alternatives||[]).some(a=>a.email==='garenright@gmail.com'),
  JSON.stringify(be));
t('...and the reason says a human has to pick',
  be.reason==='needs-recipient-review', JSON.stringify(be.reason));
