...
//
// It is OPT-IN now, not default-on, so the gate reads "!== true"
// rather than "=== false". Sending an email to a real recruiter cannot
// be undone and it lands in an inbox that already holds every previous
// application to that company, so an unset preference must mean no.
t('followup_enabled is acted on, not decorative',
  js.includes('cfg.followup_enabled !== true'), 'nothing reads it');
t('...and an unset preference means OFF, because the send is irreversible',
  !js.includes('cfg.followup_enabled === false'),
  'the follow-up email would send on a fresh install');
