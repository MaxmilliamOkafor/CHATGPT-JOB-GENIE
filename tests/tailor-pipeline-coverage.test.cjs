...
t('a cover letter typed into a textarea also counts',
  /function pasteCoverLetterText/.test(contentJs)
    && /cover\\s\*letter/.test(contentJs), 'sites without a cover upload would fail');

// ---- 6. DOCX is what gets attached, on every platform ----------------
//
// The preference now lives in the attachment engine rather than in the
// popup's payload: the employer's own field chooses, DOCX is always
// offered first, and the alternatives are built from the same reviewed
// text so nothing stale can be sent.
const attachJs = read('document-attachments.js');
t('DOCX is preferred over PDF for parseability',
  /for \(const candidate of \[file\]\.concat\(alternatives \|\| \[\]\)\)/.test(attachJs)
    && /const usable = \[file\]\.concat\(alternatives \|\| \[\]\)/.test(attachJs),
  'PDF parses worse in most ATS, so DOCX has to be tried first');
t('PDF remains a fallback rather than being dropped',
  /function buildPdfFileFromText/.test(contentJs)
    && /function alternativeFiles/.test(contentJs),
  'a field that refuses DOCX would end the run');
