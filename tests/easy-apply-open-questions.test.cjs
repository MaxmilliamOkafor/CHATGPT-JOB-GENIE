...
{
  // The facts come from the profile now rather than from a blanket
  // default, so the profile states them. What is being asserted has not
  // changed: a screening question gets the screening answer, never a
  // marketing paragraph.
  const P2 = Object.assign({}, PROFILE, {
    willing_to_relocate: true, current_company: 'Meta', current_title: 'Engineer',
    sponsorship_required: false, work_authorized_countries: ['IE', 'US'],
  });
  const cases = [
...
// matched before must still match first.
{
  ...
  // Ticking a consent box is an action with legal weight taken in the
  // applicant's name, so it is opt-in: answered blank here, and filled
  // only when the caller passes allowConsent explicitly.
  t('  consent is not given on the applicant\'s behalf',
    C.answerFor('I agree to the terms', PROFILE) === '', 'a consent box was ticked automatically');
}
