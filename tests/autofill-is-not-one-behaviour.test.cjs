...
console.log('\nAND THE ATTACH FLOW SAYS SO OUT LOUD');
{
  ...
  // Asserted on the SHAPE of the reply rather than one spelling of it:
  // the capability has to be merged into what the popup receives for a
  // CV attach, however that line is written today.
  t('  the attach reply carries the capability',
    /describeAutofill\(\)/.test(src.slice(src.indexOf('attachDocument'), src.indexOf('attachDocument') + 4000)),
