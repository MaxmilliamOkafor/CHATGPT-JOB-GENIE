...
t('method exists', !!m);
const fn=m[0];
// The toggle is OPT-IN, so the gate reads "!== true": an unset
// preference must not send. What matters is that it is read at all and
// that it can stop a send -- an email to a recruiter cannot be recalled
// and lands beside every previous application to that company.
t('gated on the followup_enabled toggle',
  /followup_enabled/.test(fn) && /cfg\.followup_enabled !== true/.test(fn) && /return;/.test(fn), fn);
...
// Scoped to tailorDocuments. A whole-file indexOf finds the FIRST
// mention of either engine anywhere in popup.js -- including in
// unrelated methods above -- and then reports "gated behind the verdict
// panel" for a detection that is not gated behind anything.
const tailorAt = src.indexOf('  async tailorDocuments(options = {}) {');
const inTailor = (needle) => {
  const at = src.indexOf(needle, tailorAt);
  return at === -1 ? Infinity : at;
};
t('detection does not live inside the ApplyVerdict block',
  detectAt < inTailor("typeof ApplyVerdict !== 'undefined'"), 'gated behind the verdict panel');
t('detection does not live inside the qualification-threshold block',
  detectAt < inTailor("typeof QualificationThresholdEngine !== 'undefined'"), 'gated behind the threshold engine');
