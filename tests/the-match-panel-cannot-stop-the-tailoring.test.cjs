...
console.log('\nAND THE RENDERER RUNS WITH NO DOM AT ALL');
{
  ...
  if (render && gauge && chips && clean) {
    ...
    // Collaborators the renderer calls on `this`. Each is a real
    // dependency of the shipped method, so a new one appearing here is
    // the signal to add it -- not a reason for the suite to die with a
    // TypeError and report a harness gap as a product failure.
    const ctx = {
