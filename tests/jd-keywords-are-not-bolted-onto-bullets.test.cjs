...
// THE WHOLE CLASS, NOT A FUNCTION CUT OUT OF IT.
//
// This used to slice fastKeywordInjection out of popup.js by brace
// matching and run the text through `new Function`. That worked only
// while the method depended on nothing but its own arguments; the
// moment the evidence gate grew helpers, the lifted copy called
// methods that were not there and the suite died with a TypeError --
// reporting a harness limitation as a product failure. Loading the
// real class means the test exercises what actually ships.
...
const inject = (cv, keywords, missing) => popup.fastKeywordInjection(cv, keywords, missing);
...
const out = inject(CV, { all: MISSING }, MISSING);
...
console.log('\nA GROUPED SKILLS SECTION KEEPS ITS GROUPS');
{
  ...
  const o = inject(GCV, { all: REAL.concat(['SQL']) }, REAL.concat(['SQL']));
