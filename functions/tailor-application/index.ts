...
1) CREATE RESUME with these exact sections and no others:
   ...
   COVER LETTER SHAPE - FOUR PARAGRAPHS, IN THIS ORDER, EVERY TIME. A letter is never delivered with a paragraph missing, and the FIRST body paragraph is never omitted: a delivered letter began "Additionally, I mentored two junior engineers" with nothing in front of it, which tells a reader the document was assembled rather than written.
   (1) OPENING - what the candidate's background is and which part of THIS posting's work it bears on. Do NOT open by announcing the application ("I am applying for the X position at Y"): the letter already carries a "Re: Application for X" line above the salutation, so that sentence repeats what is directly above it and spends the first line a reader reads on something they already know.
   (2) PROOF - one specific thing the candidate did and what came of it, compressed to its point. This must NOT retell a CV bullet: a reviewer holds both documents, and a letter that repeats the bullets is a wasted page. Take the shape of the work, not the wording. A measured pair went out at 61% and 41% word overlap with the CV's own bullets.
   (3) THIS EMPLOYER - the specific thing about this company or its work, connected to the candidate's own, as described immediately below.
   (4) CLOSE - one sentence. Not two, and never a paragraph of thanks.
   NEVER BEGIN A PARAGRAPH WITH "Additionally", "Furthermore", "Moreover", "In addition", "Also", "Secondly", "Similarly" or "Likewise". A connective points back at something, and if the paragraph it pointed at is not there the letter reads as truncated. Write each paragraph so it stands where it is.
   BODY LENGTH: at least 150 words between the salutation and the sign-off, and at most 350. Below 150 there is no room for an opening, a proof and a close, so one of the three is missing.
